...
class EnhancedAOIResponse(BaseModel):
    ...
    bounds: Optional[Dict[str, float]] = None  # Calculated field
...
async def get_aoi_by_id(
    ...
    try:
        ...
        enhanced_aoi = EnhancedAOIResponse(
            ...
            geojson=aoi_data['geojson'],
            is_public=aoi_data.get('is_public', True),
            tags=aoi_data.get('tags', []),
            created_at=datetime.fromisoformat(aoi_data['created_at'].replace('Z', '+00:00')),
            updated_at=datetime.fromisoformat(aoi_data['updated_at'].replace('Z', '+00:00')),
            analysis_count=aoi_data.get('analysis_count', 0),
            last_analysis_at=datetime.fromisoformat(aoi_data['last_analysis_at'].replace('Z', '+00:00')) if aoi_data.get('last_analysis_at') else None,
            area_km2=area_km2,
            bounds=bounds
        )
